...
public class CourseRestController {
    ...
    public ResponseEntity<Void> createPOD(@RequestPart("course") @Valid CourseNameDTO courseNameDTO,
        ...
        if(!courseService.exists(course)) {
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "El nombre introducido ya existe");
        }
    ...
    public ResponseEntity<Void> deleteTeacherInCourse(@PathVariable Long idPod, @PathVariable Long idTeacher) {
        ...
        if(course.isPresent() && teacher.isPresent() && course.get().isTeacherInCourse(teacher.get())) {
            if (teacher.get().getEmail().equals(emailMainAdmin)) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "El administrador principal no se puede borrar del curso");
            }
            ...
            }else if (courseService.findLastCourse().get() == course.get() && teacher.get().getRoles().contains("ADMIN")) {
    ...
    public ResponseEntity<Void> deleteCourse(@PathVariable Long id) {
        ...
        if(course.isPresent()){
            ...
            boolean isLastCourse = course.get() == courseService.findLastCourse().get();
    ...
    public ResponseEntity<Void> addNewTeacherToCourse(@RequestBody @Valid TeacherJoinCourseDTO teacherRequest, @PathVariable Long id) {
        ...
        if(course.isPresent()) {
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, "La dirección de correo ya existe");
        }
        throw new ResponseStatusException(HttpStatus.NOT_FOUND," ");
    }
